...
const consumeAllEvents = async ({ jukeboxId, songId }) => {
  ...
  // Exclure les priorités gratuites (is_free = 1) du calcul de totalAmount
  // Les priorités gratuites ne doivent pas générer de distribution de revenus
  const paidEvents = rows.filter((row) => row.is_free !== 1);
  const totalAmount = paidEvents.reduce((sum, row) => sum + Number(row.amount ?? 0), 0);
  
  // Utiliser le dernier événement payant pour déterminer le payerUserId
  // Si aucun événement payant, utiliser le dernier événement (gratuit)
  const lastPaidEvent = paidEvents.length > 0 ? paidEvents[paidEvents.length - 1] : rows[rows.length - 1];
  const lastEvent = rows[rows.length - 1];
  
  // Récupérer les utilisateurs qui ont payé avec leur investissement restant
  // (uniquement parmi les événements payants)
  const usersWithRemainingInvestment = paidEvents
  ...
  // utiliser l'investisseur original (investor_user_id), sinon utiliser le dernier événement payant
  let payerUserId = null;
  const eventsWithRemainingInvestment = paidEvents.filter(
  ...
  } else if (paidEvents.length > 0) {
    // Utiliser le dernier événement payant
    payerUserId = lastPaidEvent.user_id ? Number(lastPaidEvent.user_id) : null;
  } else {
    // Aucun événement payant, pas de payerUserId (priorité gratuite uniquement)
    payerUserId = null;
  }
