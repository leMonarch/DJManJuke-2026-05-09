...
const registerJukeboxSockets = (httpServer) => {
  ...
  const io = new Server(httpServer, {
    ...
    // 1. Si Passenger retire /api : Node.js reçoit /socket.io
    // 2. Si Passenger garde /api : Node.js reçoit /api/socket.io
    // On utilise /api/socket.io car c'est ce que le client envoie
    // Si Passenger retire /api, Socket.io ne trouvera pas la route, mais on essaie quand même
    path: '/api/socket.io',
    cors: {
    ...
    // Configuration pour Passenger qui peut avoir des problèmes avec WebSocket
    pingTimeout: 60000, // Timeout plus long pour Passenger
    pingInterval: 25000, // Ping plus fréquent pour maintenir la connexion
  });
  ioInstance = io;
