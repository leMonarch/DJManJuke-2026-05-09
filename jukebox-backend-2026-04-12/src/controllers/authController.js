...
const createProCheckoutSession = async (req, res, next) => {
  try {
    ...
    const session = await createProSubscriptionCheckoutSession({ userId: req.user.user_id });
    return res.status(201).json(session);
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error('[createProCheckoutSession] Error:', error.message, error.statusCode);
